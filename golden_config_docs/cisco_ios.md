...
- **SNMP User Normalization**:
  ...
  - Parses the output using a platform-specific TextFSM template (`cisco_ios_show_snmp_user.textfsm`).
...
- **TextFSM Template**: Ensure that the `textfsm_templates/cisco_ios_show_snmp_user.textfsm` file is present and correctly defines how `show snmp user` output should be parsed.
...
- **Scope**: This dispatcher is specifically designed for **Cisco IOS platforms** that require special handling for SNMP user configurations. For other platforms, the generic `NetmikoDefault` dispatcher may be more appropriate.

## File & Class Reference

- **Class**: `NetmikoCiscoIos`
...
- **TextFSM Template**:
  - `textfsm_templates/cisco_ios_show_snmp_user.textfsm`: Defines the parsing rules for `show snmp user` output.
