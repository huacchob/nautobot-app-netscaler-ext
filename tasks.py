...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NETSCALER_EXT_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.netscaler_ext.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.netscaler_ext.compose_http_timeout,
        "NAUTOBOT_VER": context.netscaler_ext.nautobot_ver,
        "PYTHON_VER": context.netscaler_ext.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.netscaler_ext.project_name}",
        f'--project-directory "{context.netscaler_ext.compose_dir}"',
    ]

    for compose_file in context.netscaler_ext.compose_files:
        compose_file_path = os.path.join(context.netscaler_ext.compose_dir, compose_file)
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.netscaler_ext.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.netscaler_ext.python_ver
    dockerfile_path = os.path.join(context.netscaler_ext.compose_dir, "Dockerfile")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations netscaler_ext"
...
@task(
    help={
        "version": "Version of Netscaler Ext to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} netscaler_ext"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("netscaler_ext") / Path("migrations")
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.netscaler_ext.local):
